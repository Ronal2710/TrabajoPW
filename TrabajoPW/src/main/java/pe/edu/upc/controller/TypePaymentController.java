...
public class TypePaymentController {
	...
	public String searchTypePayments(Model model, @Validated TypePayment TypePayment) throws ParseException {
		List<TypePayment> listTypePayments;
		model.addAttribute("typePayment", new TypePayment());
		listTypePayments = pS.search(TypePayment.getNameTypePayment());
		if (listTypePayments.isEmpty()) {

			model.addAttribute("mensaje", "No se encontró");
		}
		model.addAttribute("listTypePayment", listTypePayments);
		return "TypePayment/listTypePayment";
